/**
 * @file plane.c
 * @brief Plane geometry and intersection implementation
...
Plane plane_create_xz(float y, Color color) {
    ...
    Vec3 normal = vec3_create(0.0f, 1.0f, 0.0f);  // Up vector
...
bool plane_hit(const Hittable *hittable, const Ray *ray, 
    ...
    // Substituting: dot(normal, origin + t*direction - point) = 0
    // Solving for t: t = dot(normal, point - origin) / dot(normal, direction)
...
bool plane_point_in_front(const Plane *plane, Vec3 point) {
    return plane_distance_to_point(plane, point) > 0;
}
