...
Camera camera_create_orthographic(Vec3 origin, Vec3 target, Vec3 up,
    ...
    Vec3 w = vec3_normalize(vec3_sub(origin, target));  // Back
    Vec3 u = vec3_normalize(vec3_cross(up, w));         // Right
    Vec3 v = vec3_cross(w, u);                          // Up
...
Camera camera_create_perspective(Vec3 origin, Vec3 target, Vec3 up,
    ...
    // Lower-left corner (1 unit away from camera)
    Vec3 half_horizontal = vec3_scale(camera.horizontal, 0.5f);
    Vec3 half_vertical = vec3_scale(camera.vertical, 0.5f);
...
Ray camera_get_ray(const Camera *camera, float u, float v) {
    ...
    Vec3 target_point = vec3_add(vec3_add(camera->lower_left, horizontal_offset), vertical_offset);
    Vec3 direction = vec3_sub(target_point, camera->origin);
